    ...
    rl.question('Email: ', (email) => {
      rl.question('Password: ', async (password) => {
        try {
          ...
          await logMessage(`Error during user ID retrieval: ${error.response ? error.response.data : error.message}`);
        } finally {
          rl.close();
        }
      });
...
async function main() {
  ...
  if (!userId) {
    ...
    console.log("Reconnecting with user ID:", userId);
    await logMessage(`Reconnecting with user ID: ${userId}`);
    await connectWebSocket(userId);
  }
}
