...
const colors = ['\x1b[31m', '\x1b[32m', '\x1b[33m', '\x1b[34m']; // Definisi beberapa warna
let sockets = []; // Array untuk menyimpan semua socket
...
async function connectWebSocket(userId, proxy, account, accountIndex) {
    ...
    socket.onopen = () => {
        ...
        displayAccountStatus(account, accountIndex, 'Terhubung');
    };
    ...
    socket.onmessage = (event) => {
        ...
        // Update poin akun
        account.pointsToday = data.pointsToday; // Update objek akun
        account.pointsTotal = data.pointsTotal; // Update objek akun

        // Tampilkan status akun dengan poin yang diperbarui
        displayAccountStatus(account, accountIndex);
    };
...
function displayAccountStatus(account, accountIndex, websocketStatus) {
    const currentColorIndex = accountIndex % colors.length;
    const elapsedTime = calculateElapsedTime(); // Hitung waktu aktif
    const elapsedTimeFormatted = new Date(elapsedTime * 1000).toISOString().substr(11, 8); // Format HH:mm:ss
    ...
    console.log(`${colors[currentColorIndex]}TIME RUN  : ${elapsedTimeFormatted}\x1b[0m`);
...
let startTime = Date.now(); // Waktu mulai

function calculateElapsedTime() {
    return Math.floor((Date.now() - startTime) / 1000); // Hitung waktu dalam detik
}
